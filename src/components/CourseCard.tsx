...
export default function CourseCard() {
  ...
  return (
    <article
      ...
      "
    >
      {/* Overlay: hace clicable TODO el card, sin tap highlight ni bordes cuadrados */}
      <Link
        href="/creator-lab"
      ...
      {/* Glow sutil dentro del radio correcto (evita esquinas cuadradas) */}
      <span
        aria-hidden
      ...
      <div className="relative z-20 grid gap-4 md:grid-cols-[auto_1fr_auto] md:items-center">
        {/* Icono estrella: sin 'tap highlight' y con borde redondeado real */}
        <div className="flex md:justify-center">
          <div className="h-10 w-10 md:h-12 md:w-12 grid place-items-center rounded-2xl
                          bg-white/5 ring-1 ring-white/10 overflow-hidden
        ...
        <div className="min-w-0">
          ...
          {/* Badges compactas y en fila */}
          <div className="mt-3 sm:mt-4 flex flex-wrap items-center gap-2">
            {features.map((label) => (
              <span
                key={label}
                className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.02]
                           px-3 py-1 text-[11px] sm:text-xs text-slate-300 ring-1 ring-white/[0.04]"
              >
                {label}
              </span>
            ))}
          ...
          {/* CTA móvil: centrado y a todo el ancho (el overlay ya gestiona el click) */}
          <div className="mt-4 md:hidden">
            <div
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-white
                         ...
                         ring-1 ring-white/10 shadow-[0_10px_30px_rgba(2,132,199,0.18)] select-none"
            >
        ...
        {/* Precio + CTA desktop (el overlay captura el click) */}
